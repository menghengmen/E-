...
//                  .easingSet(chartAnimationType))//设置启用的绘制图表的动画类型
    ...
    .styleSet(AAObject(AAStyle)
              .colorSet(@"#ffffff")//标题颜色
              .fontSizeSet(@"0"));//标题字体大小
    ...
    if (chartModel.pointHollow == YES) {
        marker.fillColorSet(@"#219cfe")//点的填充色(用来设置折线连接点的填充色)
        .lineWidthSet(@(-1))//外沿线的宽度(用来设置折线连接点的轮廓描边的宽度)
        .lineColorSet(@"#ffffff")//外沿线的颜色(用来设置折线连接点的轮廓描边颜色，当值为空字符串时，默认取数据点或数据列的颜色。原点的边缘色)
        ;
    }
